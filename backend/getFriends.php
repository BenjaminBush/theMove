<?php
    ...
    $stmt = $mysqli->prepare("SELECT users.first_name, users.last_name, users.username, connections.status FROM connections INNER JOIN users ON (connections.member1_id = $current_user WHERE connections.member2_id = users.user_id) OR (connections.member2_id = $current_user WHERE connections.member1_id = users.user_id)");
    if (!$stmt) {
